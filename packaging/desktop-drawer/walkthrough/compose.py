...
F_BIG = font('BricolageGrotesque-Bold.ttf', 88)
F_MID = font('BricolageGrotesque-Bold.ttf', 44)
F_CAP = font('BricolageGrotesque-Bold.ttf', 30)
F_BODY = font('WorkSans-Regular.ttf', 28)
...
def footage(t):
    ...
    mask = mx.point(lambda v: 255 if v == 0 else 0)
...
def intro_frame(t, dur):
    ...
    if t < 1.6:
        ...
        _headline(d, 'Your desktop, again.', a)
    elif t > 2.4:
        a = int(255 * ease((t - 2.4) / 0.5))
        _headline(d, 'Enjoy your wallpaper.', a, sub='Files stay one click away.')
    return canvas
...
def _headline(d, text, a, sub=None):
    ...
    x = (W - tw) / 2; y = 330
    d.rounded_rectangle((x - 36, y - 24, x + tw + 36, y + 100 + (60 if sub else 0)), radius=22, fill=(26, 42, 70, int(a * 0.72)))
    d.text((x, y), text, font=F_BIG, fill=(255, 255, 255, a))
    if sub:
        sw = d.textlength(sub, font=F_BODY); d.text(((W - sw) / 2, y + 104), sub, font=F_BODY, fill=(255, 226, 200, a))


def lower_third(canvas, text, a=255):
    d = ImageDraw.Draw(canvas, 'RGBA')
    f = F_CAP; tw = d.textlength(text, font=f)
    x, y = 60, H - 150
    d.rounded_rectangle((x, y, x + tw + 56, y + 62), radius=10, fill=(26, 42, 70, int(a * 0.88)))
    d.rectangle((x, y, x + 8, y + 62), fill=ACCENT + (a,))
    d.text((x + 28, y + 14), text, font=f, fill=(255, 255, 255, a))
    return canvas
...
def outro_frame(t, dur):
    ...
    d = ImageDraw.Draw(canvas)
    a = ease(t / 0.6)
    lines = [('Desktop Drawer', F_BIG), ('Free. Open source. Cinnamon 6.', F_MID),
             ('Download the 1.2.0 beta from GitHub', F_BODY), ('implantintelligence.com/p/desktop-drawer', F_BODY)]
    y = 300
    for text, f in lines:
    ...
    small = 'Recorded in a real Cinnamon session with sample files. Music: Wallpaper by Kevin MacLeod, CC BY 4.0.'
...
SEGMENTS = [
    ('desktop', 'menu-open', 1.0, 'Hover the panel icon. The drawer opens.'),
    ('menu-open', 'projects', 1.0, 'Folders expand in place.'),
    ('projects', 'reading', 1.0, 'Files open in their usual app.'),
    ('reading', 'escape', 1.15, 'Keyboard too: arrows, Enter, Escape.'),
    ('escape', 'settings', 1.2, 'Or click. Choose folder opens the settings.'),
    ('settings', 'settings-closed', 1.4, 'Point it at any folder. Hover on or off, your delay.'),
    ('settings-closed', 'light-menu', 1.1, 'Follows your theme, light or dark.'),
    ('light-menu', 'end', 1.0, ''),
]
INTRO = 4.6; OUTRO = 4.4; WIPE = 0.5
...
cues.append((0.0, INTRO, 'Your desktop, again. Enjoy your wallpaper. Files stay one click away.'))
...
cues.append((outro_start, frame_idx / FPS, 'Desktop Drawer. Free, open source, for Cinnamon 6. Download the 1.2.0 beta from GitHub.'))
